...
class VGG8(nn.Module):
    def __init__(self,inplace):
        ...
        self.maxp = nn.MaxPool3d(kernel_size=2, stride=2)
...
class GlbalAttention(nn.Module):
    def __init__(self, transformer_num_heads=8, hidden_size=512, transformer_dropout=0.0):
        super().__init__()
        ...
        self.proj_dropout = nn.Dropout(transformer_dropout)
...
class GlobalLocalBrainAge(nn.Module):
    def __init__(self, inplace, patch_size=64, step=1, nblock=6, drop_rate=0.5, backbone='vgg8'):
        super().__init__()
        ...
        if backbone == 'vgg8':
            ...
            raise ValueError('%  model dose not supported:'%backbone)
        
        self.attnlist = nn.ModuleList()
        self.fftlist = nn.ModuleList()
        
        for n in range(nblock):
            atten = GlbalAttention(transformer_num_heads=8, hidden_size=hidden_size, transformer_dropout=drop_rate)
    ...
    def forward(self, xinput):    
        ...
        xgfeat = self.avg(xglo)
        xgfeat = torch.flatten(xgfeat, 1)
        ...
        xglot = xglot.permute(0, 2, 1).contiguous()
        for z in range(0, D-self.patch_size, self.step):
            for y in range(0, W-self.patch_size, self.step):
                for x in range(0, H-self.patch_size, self.step):
                    locx = xinput[:, :, x:x+self.patch_size, y:y+self.patch_size, z:z+self.patch_size]
                    xloc = self.local_feat(locx)
                
                    for n in range(self.nblock):
                        B1, C1, H1, W1, D1 = xloc.size()
                        xloct = xloc.view(B1, C1, H1*W1*D1)
                        xloct = xloct.permute(0, 2, 1).contiguous()
                    
                        tmp = self.attnlist[n](xloct, xglot)
                        tmp = tmp.permute(0, 2, 1).contiguous()
...
if __name__ == '__main__':
    ...
    x1 = torch.rand(4,1, 91,109,91).to(device)
    y1 = torch.rand(4,1).to(device)
    criterion = torch.nn.L1Loss()
    
    mod = GlobalLocalBrainAge(inplace=1,
                              patch_size=32,
                              step=32,
                              nblock=8,
                              backbone='vgg8')
    ...
    loss_list = []
    # print(zlist)
    tmp_loss = torch.Tensor(0).to(device)
    
    for z in zlist:
        # print(z.shape)
        loss = criterion(z, y1)
        # tmp_loss = loss + tmp_loss
